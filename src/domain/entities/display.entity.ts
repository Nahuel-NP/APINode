export class DisplayEntity {
  ...
  static fromObject(object: { [key: string]: any }): DisplayEntity {
    const { id, name, description, picture_ur, user_id, price_per_day, resolution_height, resolution_width, type } = object;

    if (!id) throw new Error('Id is required');
    if (!name) throw new Error('Name is required');
    if (!description) throw new Error('Description is required');
    if (!picture_ur) throw new Error('Picture is required');
    ...
    return new DisplayEntity(id, name, description, picture_ur, user_id, price_per_day, resolution_height, resolution_width, type);
  }
}
